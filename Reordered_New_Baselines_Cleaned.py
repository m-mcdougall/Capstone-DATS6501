...
for state in tqdm(reviews_df.State.unique()):
    subset_state = (reviews_df[reviews_df.State == state]).sample(n=3000, replace=False, random_state=42)
    collect_sampler.append(subset_state)
...
def add_all_features(df_in, save_name=''):
    ...
    df_in.to_csv(wd+'\\Data\\Cleaned\\Split\\New_Samples_reordered_3000_'+save_name+'.csv')
